...
public class Copier {
	...
	public void treatQueue() {
		...
		while (!this.mainQueue.isEmpty()) {
			...
			if (destFile.length() > 0) { //target exists
				...
				if(actionToDo == -1){
				 actionToDo = Configuration.collisionAction;
				}
				...
				while (!acceptableAction) {
					...
					case (0) : //ask
						if(choice == -1){
						 choice = DialogMsg.alreadyExistsChoice(
 ...
 public void setChoice(int choiceNumber,int actionNumber){
	 this.choice = choiceNumber;
	 this.actionToDo = actionNumber;
 }
